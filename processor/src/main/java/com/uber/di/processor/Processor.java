 ...
 * Foo_Bar.dot}.
 */
public final class Processor implements BindingGraphPlugin {
    private Filer filer;
    ...
    public void visitGraph(BindingGraph bindingGraph, DiagnosticReporter diagnosticReporter) {
        ...
        ClassName componentName = className(componentElement);
    ...
    static class ClassName {
        ...
        ClassName(String packageName, String... simpleNames) {
            this.packageName = packageName;
            this.simpleNames = Arrays.asList(simpleNames);
        }

        ClassName nestedClass(String name) {
            ImmutableList<String> simpleNames = ImmutableList.<String>builder()
                    .addAll(this.simpleNames)
                    .add(name)
                    .build();
            return new ClassName(packageName, simpleNames.toArray(new String[0]));
        }
    }

    static ClassName className(TypeElement element) {
        ...
        return element.getEnclosingElement().accept(new SimpleElementVisitor8<ClassName, Void>() {
            ...
            public ClassName visitPackage(PackageElement packageElement, Void p) {
                return new ClassName(packageElement.getQualifiedName().toString(), simpleName);
            }

            @Override
            public ClassName visitType(TypeElement enclosingClass, Void p) {
                return className(enclosingClass).nestedClass(simpleName);
            }
